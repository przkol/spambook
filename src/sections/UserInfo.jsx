...
const UserInfo=()=>{
    ...
    const registrationDate= new Date(userInfo?.registered?.date)
...
const handleInfoInput=(e)=>{
    ...
    switch(targetInputInfo){
        ...
        case('nameFirst'):
            setUserDetails(prevState=>({...prevState,
            name:{first:targetInputValue,
            last:prevState.name.last}
            }))
            break;
        case('nameLast'):
            setUserDetails(prevState=>({...prevState,
            name:{last:targetInputValue,
...
if(mainUser.loaded){
    ...
    <StyledUserInfo className={mainUser.loaded ? 'userInfo active' : 'userInfo'}>
        ...
        <div className='baseInfo'>
            ...
            <div><p>First name:</p>{edittingBaseInfo ? <input info='nameFirst'value={userDetails.name.first} onChange={handleInfoInput} type='text' /> :<p>{userInfo.name.first}</p>}</div> 
            <div><p>Last name:</p>{edittingBaseInfo ? <input info='nameLast'value={userDetails.name.last} onChange={handleInfoInput} type='text' /> :<p>{userInfo.name.last}</p>}</div> 
            <div><p>Registered:</p><p>{registrationDate.toLocaleDateString()} ({userInfo.registered.age} yrs ago)</p></div> 
            <div><p>Date of Birth:</p><p>{birthDate.toLocaleDateString()} ({userInfo.dob.age} y/o)</p></div> 
            </div>
        <div className='contactInfo'>
            ...
            <div><p>E-mail address</p>{edittingContactInfo ? <input info='email'value={userDetails.email} onChange={handleInfoInput} type='email'/> :<p>{userInfo.email}</p>}</div> 
        ...
        <div className='addressInfo'>
            ...
            <><input info='location' info2='name' value={userInfo.location.street.name} onChange={handleInfoInput} type='text' />
            <input info='location' info2='number' value={userInfo.location.street.number} onChange={handleInfoInput} type='number' /> </>    
            :<p>{userInfo.location.street.name} {userInfo.location.street.number}</p>}</div> 
            <div><p>City:</p>{edittingAddressInfo ? <input info='location' info2='city' value={userDetails.location.city} onChange={handleInfoInput} type='text' /> :<p>{userInfo.location.city}</p>}</div> 
            <div><p>State:</p>{edittingAddressInfo ? <input info='location' info2='state' value={userDetails.location.state} onChange={handleInfoInput} type='text' /> :<p>{userInfo.location.state}</p>}</div> 
            <div><p>Postcode:</p>{edittingAddressInfo ? <input info='location' info2='postcode' value={userDetails.location.postcode} onChange={handleInfoInput} type='text' /> :<p>{userInfo.location.postcode}</p>}</div> 
            <div><p>Country:</p>{edittingAddressInfo ? <input info='location' info2='country' value={userDetails.location.country} onChange={handleInfoInput} type='text' /> :<p>{userInfo.location.country}</p>}</div> 
            </div>
